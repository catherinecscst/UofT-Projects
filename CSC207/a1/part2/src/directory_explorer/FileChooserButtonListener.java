...
public class FileChooserButtonListener implements ActionListener {
	...
	private static void buildTree(File file, FileNode curr) {
		// Check if the root fileNode has children.
		if (!file.listFiles().equals(null)) {
			for (File unknown: file.listFiles()){
				if (unknown.isDirectory()){
					FileNode child = new FileNode(unknown.getName(), curr, FileType.DIRECTORY);
					curr.addChild(unknown.getName(), child);
					// Because it is Directory, so continue searching.
					buildTree(unknown, child);
				} else {
					FileNode child = new FileNode(unknown.getName(), curr, FileType.FILE);
					curr.addChild(unknown.getName(), child);
				}
			}
		}
	...
	private static void buildDirectoryContents(FileNode fileNode, StringBuffer contents, String prefix) {
		...
		for (FileNode child: fileNode.getChildren()){
			String subprefix = prefix + DirectoryExplorer.PREFIX;
			if (child.isDirectory()){
				buildDirectoryContents(child, contents, subprefix);
			} else {
				contents.append(subprefix + child.getName() + "\n");
			}
		}
	}
}
