...
class PartialParse(object):
    ...
    def parse_step(self, transition_id, deprel=None):
        ...
        if transition_id == self.left_arc_id:
            if len(self.stack) == 1:
                raise ValueError('Something bad happened: no dependency can be done on ROOT')
            elif len(self.stack) == 2:
                raise ValueError('Something bad happened: cannot call LEFT-ARC on ROOT')
            ...
            if self.next == len(self.sentence):
                raise ValueError('Something bad happened: buffer is empty.')
            self.stack.append(self.next)
            self.next += 1
        else:
            raise ValueError('Something bad happened: transition_id is not valid.')
        # *** END YOUR CODE ***
    ...
    def get_n_leftmost_deps(self, sentence_idx, n=None):
        ...
        count = 0
        if len(self.arcs) == 0:
            raise ValueError("Something bad happened: no arcs.")
        for deprel in self.arcs:
            if count == n:
                break
            elif deprel[0] == sentence_idx:
                deps.append(deprel[1])
                count += 1
        # if len(deps) < n:
        #     return deps
    ...
    def get_oracle(self, graph):
        ...
        *ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
        ...
        if len(self.stack) == 1:
            # [ROOT] Already know that the buffer is not empty.
            transition_id = self.shift_id

        elif graph.nodes[self.stack[-2]]['head'] == self.stack[-1]: # require left arc
            transition_id = self.left_arc_id
            if graph.nodes[self.stack[-2]]['rel'] in graph.nodes[self.stack[-1]]['deps']:
                deprel = graph.nodes[self.stack[-2]]['rel']

        elif graph.nodes[self.stack[-1]]['head'] == self.stack[-2]: # require right arc
            grd = list(get_right_deps(graph.nodes[self.stack[-1]]))
            if len(grd) == 0 or self.next > max(grd):
                transition_id = self.right_arc_id
                if graph.nodes[self.stack[-1]]['rel'] in graph.nodes[self.stack[-2]]['deps']:
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        predicitions = model.predict(minibatch)
        # Perform a parse step on each partial parse in the minibatch with its predicted transition;
        for i in range(len(minibatch)):
            partial_parse = minibatch[i]
            predicition = predicitions[i]
            try:
                partial_parse.parse_step(predicition[0], predicition[1])
