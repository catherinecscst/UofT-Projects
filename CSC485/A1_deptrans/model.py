...
class ParserModel(nn.Module):
    ...
    def create_embeddings(self, word_embeddings):
        ...
        # Create 3 embedding (2D) tensors, one for each of the input types
        # The word embedding tensors should be initialized with the value of the argument word_embeddings;
        self.word_embeddings = torch.tensor(word_embeddings, requires_grad=True)
        # The other two matrices should be initialized using the He initializer you implemented.
    ...
    def embedding_lookup(self, id_batch, n_ids, embedding_matrix):
        ...
        # id_batch is converted to one-hot vectors first
        one_hot_vectors = one_hot_float(id_batch, n_ids)        # shape=(B, N, n_ids)
        # then multiply with the embedding matrix
        lookup_results = embedding_matrix * one_hot_vectors     # shape=(B, N, embed_size)
        # Reshape this embedded tensor into shape (B, N * embed_size)
        embedded_batch = torch.reshape(lookup_results, (lookup_results.size()[0], -1))  # shape=(B, N * embed_size)
    ...
    def get_concat_embeddings(self, word_id_batch, tag_id_batch,
        ...
        embedded_word_batch = self.embedding_lookup(word_id_batch, self.config.n_word_ids, self.word_embeddings)
        embedded_tag_batch = self.embedding_lookup(tag_id_batch, self.config.n_tag_ids, self.tag_embeddings)
        embedded_deprel_batch = self.embedding_lookup(deprel_id_batch, self.config.n_deprel_ids, self.deprel_embeddings)
        # embedded_x.x_batch shape=(B, {n_word_features/n_tag_features/n_deprel_features} * embed_size)
        # Concatenate columns in this case:
        x = torch.cat([embedded_word_batch, embedded_tag_batch, embedded_deprel_batch], 1)
    ...
    def forward(self, word_id_batch, tag_id_batch, deprel_id_batch):
        ...
        # h = Relu(x W_h + b_h)
        h = F.relu(x * self.W_h + self.b_h)
        # h_drop = Dropout(h, dropout_rate)
        h_drop = F.dropout(h, p=self.dropout, training=self.training)
        # pred = h_drop W_o + b_o
        pred = h_drop * self.W_o + self.b_o
    ...
    def add_optimizer(self):
        ...
        self.optimizer = torch.optim.Adam([self.word_embeddings, self.tag_embeddings, self.deprel_embeddings, 
