...
(define (deep-map-once f v)
  (cond [(and (equal? (f v) v) (list? v)) (map (λ (i) (deep-map-once f i)) v)]
        [(and (equal? (f v) v) (not (list? v))) v]
        [(not (equal? (f v) v)) (f v)])
  )

;(define (deep-map-once f v)
;  (cond [(not (equal? (f v) v)) (f v)]
;        [(list? v) (map (λ (i) (deep-map-once f i)) v)]
;        [else v])
;  )
...
(module+ test
  (check-equal? (If-rule '(If a b c))
                '(a (Lambda () b) (Lambda () c)))
  (check-equal? (expand If-rule '(If a (If g h u) b))
                '(a
                  (Lambda
                   ()
                   (g (Lambda () h) (Lambda () u)))
