...
(define (adjacent spot)
  (-< (map + (identity spot) ((-< identity reverse) (list 0 1)))
      (map - (identity spot) ((-< identity reverse) (list 0 1))))
  )
...
(define (assert b)
  (if (equal? #false b) (fail) #true)
  ;   (cond [(equal? #false b) (fail)]
  ;         [else #true])
  )
;(list 2 (assert #true) 3)
...
(define (then-assert condition body ...)
  (cond [(equal? condition #false) (fail)]
        [else body ...]))
...
(define (path to through from [partial? #true])
    (cond [(equal? through (memv from through)) '()]
           [(equal? to from) '(())]
           [else (append (list from) (list (-< (adjacent from))))]
           ;[else (append (list from) (path to (remove from through) (-< (adjacent from))))]
           )
...
(path (list 0 1) figure-eight (list 0 0))
(next)
;(list-results (path (list 2 4) figure-eight (list 0 0)))
