...
(define (without element a-list)
  (local [(define (not-same? each_num)
            (not(equal? each_num element)))]
    (filter not-same? a-list)))
...
(define (paths to through from)
  (local [(define (a-p new-from)
            (cond [(equal? new-from to) (list (list to))]
                  [(not (not-in? new-from (without from through))) (paths to (without from through) new-from)]
                  [else (list)]))]
    (cond [(not-in? from through) (list)]
          [(equal? from to) (list(list to))]
          [else (include-in-all from (apply append(map a-p (adjacent from))))])))
