...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    ...
    if(syscall < 0 || syscall > NR_syscalls-1 || syscall == MY_CUSTOM_SYSCALL){
    	...
    	//Check that the caller has the right permissions (-EPERM), we must be root
		if (current_uid() != 0){
			return -EPERM;
		}
		//Check for -EBUSY conditions, intercepting a system call that is already intercepted.
		...
		//Altering the sys_call_table, using the set_addr_rw/set_addr_ro functions
		...
		//if 'pid' is 0 => We want to start monitoring all
		if (pid == 0){
			...
			spin_lock(&my_table_lock);
			//All monitored already
			if (table[syscall].monitored == 2){
				spin_unlock(&my_table_lock);
				return -EBUSY;
			} else { //not all monitored yet
				table[syscall].monitored = 2;
				spin_unlock(&my_table_lock);
			}

		} else {
			//a pid does not belong to a valid process
			if (pid_task(find_vpid(pid), PIDTYPE_PID) == NULL){
				return -EINVAL; 
			} else { // a pid belongs to a valid process
				//either it is root or the 'pid' requested is owned by the calling process
				if (current_uid() == 0 || check_pids_same_owner(current->pid, pid) == 0){
					// pid not monitored yet
		...
		//if 'pid' is 0 => We want to stop monitoring all
		if (pid == 0){
			//the calling process is not root
			if (current_uid() != 0){
				//access is denied
